...
- **쓰레기·낚시**: 쓰레기는 10~20초마다 떨어지고 10개부터 물이 탁해지며 30개면 새 생물이 오지 않는다(`src/sim/trash.ts`). 낚시는 잡으면 반드시 놓아 준다(`src/sim/fishing.ts`, 잡아먹지 않는다 규칙). 둘 다 전용 난수를 써서 평소 동작·기준 기록을 건드리지 않는다. 낚시 버튼은 아래 가운데 가장자리에 두고, 낚시 게이지·글자도 그 위 아래쪽에 모아 화면 가운데를 비운다.
...
- **마우스·터치**: 왼쪽(탭)은 먹이(생물이면 그 종 먹이, 빈 곳이면 기본 가루 먹이), 오른쪽(길게 누르기)은 교감(빈 곳이면 유리 두드리기). 호버(탭한 생물)는 연노랑 테두리와 이름만 띄우고 카드·팝업은 쓰지 않는다. 키보드 기능은 터치 화면에서 오른쪽 위 ☰ 메뉴로 모은다. 모바일 UI는 안전 영역(노치) 안쪽에 두고 화면 가운데를 가리지 않는다.
- **픽셀 격자·화면 비율**: 무대는 480×270(사건 자리·가운데 비우기·자막 기준)이고, 보이는 월드는 휴대폰 가로 화면에 맞춘 588×270(무대 양옆 54px 여백)이다. 창은 588×270으로 빈틈없이 덮고(검은 여백 금지) 그보다 짧은 화면은 양옆부터 자른다. 마지막 수중 후처리가 월드 픽셀로 다시 모은다. 스프라이트는 정수 픽셀에 놓고 `Nearest`로 샘플링한다. 생물 등장·퇴장·돌아서기 가장자리는 창에 실제로 보이는 범위(`setVisibleRange`)를 따르고, 사건의 무대 밖 좌표는 `offstage`로 여백 바깥까지 민다. 배경 층은 무대 기준으로 굽고 양옆을 거울로 이어 붙여 넓힌다.
...
- **깔끔함**: 바둑판 디더링·색수차·필름 그레인·화면 전체 흔들림을 쓰지 않는다(흔들림은 열수 분출 때만). 배경 층만 물속 일렁임(`wavy.frag`)으로 흔들고 생물·소품은 흔들지 않는다. 먼 층 생물은 불투명하게 그린다(뒤가 비치면 투명해 보인다). 화면 가운데는 비워 둔다.
